...
## YouCompleteMe autotools 적용

`~/.vim/plugged/YCM-Generator/config_gen.py -b autotools .`

- 현재 폴더에 build system으로 autotools를 사용한다는 뜻
- rootDir에서 위 명령어를 실행시킨다.
- `config_gen.py`가 python2로 작성되어있기 때문에 소스코드를python3로 변경해줘야 한다.
- 아래 python3 문법에 맞게 수정한 소스코드
...
def main():
    ...
    with tempfile.NamedTemporaryFile(mode="w+", delete=False) as c_build_log:
        with tempfile.NamedTemporaryFile(mode="w+", delete=False) as cxx_build_log:
